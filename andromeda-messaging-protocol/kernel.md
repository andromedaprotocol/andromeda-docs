...
All of our ADOs have an [AMPReceive](broken-reference) execute message to handle receiving packets from the Kernel.
...
Information about the `AMPPkt` struct can be found [here](broken-reference).
...
| Name      | Type                       | Description          |
| --------- | -------------------------- | -------------------- |
| `message` | [AMPMsg](broken-reference) | The message to send. |
...
Used to save the addresses of the AMP ADOs that the Kernel will be interacting with such as the [ADODB](broken-reference), [Economics](broken-reference), and [VFS](broken-reference).&#x20;
...
{% hint style="warning" %}
This message can only be called by the [VFS](broken-reference).
{% endhint %}
...
vfs : Gets the address of the [VFS](broken-reference) ADO used by the kernel.

adodb: Gets the address of the [ADODB](broken-reference) ADO used by the kernel.

economics: Gets the address of the [Economics](broken-reference) ADO used by the kernel.
{% endhint %}
...
Checks if the specified `address` was created by an Andromeda ADO by checking and comparing the `code_Id` of the address with the code\_Ids stored in the [ADODB](broken-reference).
...
Returns a Vec<[Coin](broken-reference)> containing the recoverable funds.
