...
<table><thead><tr><th width="249.33333333333331">Name</th><th width="252.39014373716634">Type</th><th>Description</th></tr></thead><tbody><tr><td><code>validator</code></td><td>Option&#x3C;Addr></td><td>The address of the validator to claim the rewards for.</td></tr><tr><td><code>recipient</code></td><td>Option&#x3C;<a href="broken-reference">AndrAddr</a>></td><td>An optional address to receive the rewards. Will default to the sender if not specified.</td></tr></tbody></table>
...
| `amount`              | [Coin](broken-reference)      | The amount of funds locked in the delegation.                                                                                                |
| `can_redelegate`      | [Coin](broken-reference)      | `can_redelegate` captures how much can be immediately redelegated. 0 is no redelegation and `can_redelegate` == `amount` is redelegate all.  |
| `accumulated_rewards` | Vec<[Coin](broken-reference)> | The amount of rewards accruied from the staking that can be currently withdrawn.                                                             |
...
| Name        | Type                          | Description                                                                          |
| ----------- | ----------------------------- | ------------------------------------------------------------------------------------ |
| `fund`      | [Coin](broken-reference)      | The denom and amount of the token being unstaked.                                    |
| `payout_at` | [Timestamp](broken-reference) | The time these tokens can be claimed which is the time the unbonding period is over. |
