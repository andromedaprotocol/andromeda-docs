...
| ---------------- | ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `token_address`  | [AndrAddr](broken-reference) | Address of the CW20 token to be sold.                                                                                                                                                             |
...
Receives CW20 tokens sent from the `token_address` contract by performing a  [Send](broken-reference). These tokens can be set for sale or used to purchase other CW20 tokens. This depends on the attached hook message.
...
| `start_time`    | [Expiry](broken-reference)               | Optional start time in milliseconds since [epoch](https://www.epochconverter.com/clock). If not specified, then the sale will start immediately. |
| `duration`      | [MillisecondsDuration](broken-reference) | Optional duration for the sale in milliseconds from the `start_time`. If not specified then the sale never expires.                              |
...
| `start_time`    | [Expiration](broken-reference) | The time when the sale starts.                                                |
| `end_time`      | [Expiration](broken-reference) | The time when the sale ends.                                                  |
