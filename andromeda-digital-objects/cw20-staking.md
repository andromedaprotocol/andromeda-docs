...
This **CW20 Staking ADO** allows users to stake a specified [CW20 token](broken-reference) and to receive rewards in any number of other tokens in proportion to their share. The reward token does not need to be the token they stake with but it can be. The contract allows for two types of rewards:
...
| `init_timestamp`    | [Expiry](broken-reference)                                   | Timestamp from which Rewards will start getting accrued against the staked LP tokens. Specified in milliseconds. |
...
| Name              | Type                                                         | Description                                                                                                         |
| ----------------- | ------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------- |
| `till_timestamp`  | [Expiry](broken-reference)                                   | Timestamp in milliseconds till which rewards will be accrued. No staking rewards are accrued beyond this timestamp. |
| `cycle_rewards`   | Uint128                                                      | Rewards distributed during the 1st cycle.                                                                           |
| `cycle_duration`  | [Milliseconds](broken-reference)[Duration](broken-reference) | Cycle duration in milliseconds.                                                                                     |
...
The rest of the executes can be found in the [`AndrReceive`](broken-reference) section.
...
<table><thead><tr><th width="306.970342910102">Name</th><th width="179.33333333333331">Type</th><th>Description</th></tr></thead><tbody><tr><td><code>staking_token</code></td><td><a href="broken-reference">AndrAddr</a></td><td>The token accepted for staking.</td></tr><tr><td><code>number_of_reward_tokens</code></td><td>u32</td><td>The current number of reward tokens, cannot exceed the maximum of 10.</td></tr></tbody></table>
...
A set of base queries common to all Andromeda ADOs. Check[ AndrQuery](broken-reference).
