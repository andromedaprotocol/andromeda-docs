...
If unfamiliar with the steps of deploying an app, go back to the [first example](broken-reference) where we explain in detail all the different parts of deploying an App.
...
Make sure to read the [Introduction to Apps](broken-reference) before going through building an app.
{% endhint %}
...
* [CW20](broken-reference)
* [CW20 staking](broken-reference)
...
If you do not have the CLI downloaded, go to the [Introduction to Apps](broken-reference) section to get the latest version.
...
As we can see each of the stakers got the amount proportional to the amount staked. Each user can now claim these tokens by calling the [ClaimRewards](broken-reference) message.
